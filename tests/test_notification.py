...
def notification_service(mock_speaker_service, mock_tts_service):
    ...
    with (
        patch("src.services.notification.TTSService", return_value=mock_tts_service),
...
class TestNotificationService:
    ...
    async def test_send_github_notification_success(
        self, notification_service, mock_speaker_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_github_notification_failure(
        self, notification_service, mock_speaker_service
    ):
        """Test sending GitHub notification for failure."""
        result = await notification_service.send_github_notification(
            repo="user/repo", workflow="CI", conclusion="failure"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_message_with_piper_tts(
        self, notification_service, mock_speaker_service, mock_tts_service
    ):
        """Test _send_message uses Piper TTS when configured."""
        with patch("src.services.notification.settings") as mock_settings:
            mock_settings.piper_voice_zh = "zh_CN-huayan-medium"
            mock_settings.get_static_server_url = MagicMock(
                return_value="http://192.168.1.100:1810"
            )
